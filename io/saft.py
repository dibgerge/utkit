...
def saft(fname):
    """
    Reads a file stored in SAFT format.

    Parameters:
        fname (string):
            Name of the file to open (with absolute or relative path).
    Returns:
         :class:`utkit.RasterScan`, header:
            A 2-element tuple where the first element is the RasterScan stored in the SAFT file,
            returned as a :class:`utkit.RasterScan` object. The second element is a dictionary
    ...
    return ut.RasterScan(data, items=Y, major_axis=t, minor_axis=X), header
