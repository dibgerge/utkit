...
def _readData(fid, fmt, Addr, datalen, commtype=0):
    fid.seek(Addr)
    fm = fmt + str(int(datalen/2)) + ('h' if commtype else 'b')
    nread = datalen if commtype else int(datalen/2)
    data = fid.read(nread)
    return np.array(unpack(fm, data))
    # fmt = fmt + str(datalen) + "b"
    # nbytes = struct.calcsize(fmt)
    # data = fid.read(nbytes)
    # result = np.frombuffer(data, dtype='int16')
    # return result
